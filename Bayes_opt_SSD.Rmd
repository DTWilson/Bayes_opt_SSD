...
The recommended evaluation is at $n=$ `sol[1]`, $k=$ `sol[2]`. Evaluating the point using $N=100$ MC samples gives an estimated type II error rate of `y[1]` (sd `sqrt(y[2])`), so it appears to be feasible. We can add the new information to our records, update the GP model, and then use its predictions to confirm whether or not the new point can join the approximation set.
...
colnames(tab2) <- c("$2n$", "$k$", "$j$", "$\\beta$ (s.e.), $N = 10^2$", "$\\beta$ (s.e.), $N = 10^4$")
tab2
...
tab2 <- data.frame(n_1=tab$n_1, n=tab$n_1*(1+tab$r), k=tab$k, j=tab$k*2, a=tab$a,
...
#  & & & & & \multicolumn{2}{c}{$N = 10^2$} & \multicolumn{2}{c}{$N = 10^4$} \\
...
data_set <- "13_12_17"
pf <- readRDS(paste0("../R/CC/Data_",data_set,"/p_front.Rda"))

pf<- pf[pf$f2 <= 30,]
pf <- rbind(pf, c(1200, 7))
pf$t <- "p"
pf <- unique(pf)

pf$f1 <- c(472, 472, 473, 474, 476, 478, 481, 483, 486, 492, 498,
...
ggplot(df, aes(f1, f2)) + geom_polygon(data=pg, alpha=0.1) + #fill="grey90") + 
  ...
  geom_point(data=data.frame(f1=1200, f2=30), shape=4) +
  annotate("text", x = 900, y = 23, label = "H(A)")
  
# ggsave("U:/Projects/MRC SDF/WP4/Papers/Bayes_opt/Figures/fake_pareto.pdf", width= 5, height = 3)
# ggsave("U:/Projects/MRC SDF/WP4/Papers/Bayes_opt/Figures/fake_pareto.eps", width= 5, height = 3, device=cairo_ps)
```
...
#ggsave("U:/Projects/MRC SDF/WP4/Papers/Bayes_opt/Figures/GP_example.eps", width= 5, height = 3.3, device=cairo_ps)
```