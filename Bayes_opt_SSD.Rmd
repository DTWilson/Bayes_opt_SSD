...
DoE <- data.frame(maximinLHS(DoE_num, dim))
...
As suggested by a reviewer, add a simple example where the exact result is known. We will look at the power of a two-sample t-test. Although we can calculate power exactly in this case, we replicate the simulation setting by adding some Monet Carlo error to the true power.
...
calc_rates4 <- function(x, h, N)
{
  ...
  mc_err <- rnorm(1, 0, sqrt(pow*(1-pow)/N))
  est <- pow + mc_err
  res <- c(1-min(max(est, 0),1), pow*(1-pow)/N)
...
## Choose initial points
DoE <- data.frame(optimumLHS(DoE_num, dim))
names(DoE) <- design_space$name
for(i in 1:dim){
  DoE[,i] <-  DoE[,i]*(design_space$up[i]-design_space$low[i]) + design_space$low[i]
}
...
Get the exact optimal sample sizes:
```{r}
...
ks <- 30:100
ns <- sapply(ks, function(x) optim(200, calc_rates4_opt, method = "Brent", lower = 10, upper = 5000,
                                   k = x, h = h)$par)

plot(ns, ks)
points(b[,1:2], col="red")
```
